...
At verification time `larp-pi.local` failed DNS resolution and did not answer a
targeted mDNS query on laptop Wi-Fi (`10.26.0.247`). ADB listed no Quest. No Pi
deployment, physical calibration, headset installation, stereo check or Quest
performance measurement is claimed for this candidate. The previous map-memory
APK is retained as `Rollback-Navigation-BeforePiSensorPullTest.apk`.
