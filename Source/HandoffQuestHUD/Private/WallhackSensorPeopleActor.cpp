...
bool AWallhackSensorPeopleActor::ViewerPose(FVector& Position, FQuat& Orientation) const
{
    ...
    const auto* Camera = PC ? PC->PlayerCameraManager : nullptr;
...
void AWallhackSensorPeopleActor::HidePeople()
{
    ...
    for (auto& Label : Labels) if (Label) Label->SetHiddenInGame(true);
}
...
void AWallhackSensorPeopleActor::Tick(float DeltaSeconds)
{
    ...
    while (Labels.Num() < Views.Num())
    {
        ...
        Label->RegisterComponent(); Labels.Add(Label);
    }
    for (int32 I = 0; I < Views.Num(); ++I)
    {
        const auto& View = Views[I]; auto* Label = Labels[I].Get();
        const FVector Position = View.Feet + FVector(0,0,1.8 * Scale);
        Label->SetText(FText::FromString(FString::Printf(TEXT("C%d %s"), View.Id, View.bRadar ? TEXT("RADAR") : TEXT("ESTIMATED"))));
        Label->SetTextRenderColor(View.bRadar ? FColor(65,230,90) : FColor(255,158,20));
        Label->SetWorldSize(.09 * Scale);
        Label->SetWorldLocationAndRotation(Position, (Viewer - Position).Rotation());
