...
bool FWallhackSensorPeopleRegistrationMathTest::RunTest(const FString&)
{
    for (const float Scale : {50.f, 100.f, 200.f})
    {
        ...
        TestTrue(TEXT("Turning right puts fixed person on left"), FMath::IsNearlyEqual(View.RightMeters, -2.f));
    }
    FVector Hit;
...
bool FWallhackSensorPeopleRendererTest::RunTest(const FString&)
{
    ...
    {
        Pose.Tint = Green;
        Renderer->Present({Pose}, INDEX_NONE, nullptr, true, Units);
        ...
        if (!TestNotNull(TEXT("Human mesh loaded"), Body->GetStaticMesh())) return false;
